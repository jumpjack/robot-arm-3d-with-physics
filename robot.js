...
******************/
    ...
    this.ji = utils.fixedJoint(
      ...
      this.base.h / 2,
      this.base.w / 2,
    ); // indicator is fixed to base
    this.j1 = utils.revoluteJoint(
      world,
      this.mast,
      this.arm_base,
      xAxis,
    ...
    this.j2 = utils.revoluteJoint(
      world,
      this.arm_base,
      this.shoulder,
      zAxis,

      this.arm_base.w / 2 + 0.1,
      0,
      0,
  ...
  updateModels() {
    ...
    for (let i in this.parts) {
      ...
      spnDebug.innerHTML +=
        i +
        ": " +
        ((pitch * 180) / 3.14).toFixed(0) +
        ", " +
        ((yaw * 180) / 3.14).toFixed(0) +
        ", " +
        ((roll * 180) / 3.14).toFixed(0) +
        "<br>";
      ...
      this.base.m.position.set(-0.2, 0, 0);

      /*  
      // Imposta la rotazione a 0 gradi intorno agli assi Y e Z
      const rotationY = 0; // Angolo di rotazione intorno all'asse Y in gradi
      const rotationZ = 0; // Angolo di rotazione intorno all'asse Z in gradi

      let qq = new THREE.Quaternion().setFromEuler(
        new THREE.Euler(
      ...
      this.parts[3].m.updateWorldMatrix(true, true);
      this.parts[3].m.getWorldQuaternion(qq);
      this.parts[3].r.setNextKinematicRotation(
        { w: qq.w, x: qq.x, y: qq.y, z: qq.z },
        true,
      );
  ...
  setPlatformTranslation(target_direction) {
    ...
    this.platform.computeColliderMovement(
      this.base.c,
      { x: d.x, y: d.y, z: d.z },
      0,
      -1,
    ...
    this.base.r.setNextKinematicTranslation(
      { x: pt.x + T.x, y: pt.y + T.y, z: pt.z + T.z },
      true,
    );
  }
  ...
  setPlatformRotation(target_rotation) {
    ...
    this.base.r.setNextKinematicRotation(
  ...
  setGripperTranslation(pointer_target) {
    ...
    this.gripper.computeColliderMovement(this.g3.c, t, 0, -1, function (c) {
      return !(c.is_robot || c.ignore_controller);
    });
    let cm = this.gripper.computedMovement();
    this.g3.r.setNextKinematicTranslation(
      { x: gt.x + cm.x, y: gt.y + cm.y, z: gt.z + cm.z },
      true,
    );
  }
  ...
  setGripperRotation(pointer_target) {
    ...
    this.g3.r.setNextKinematicRotation(
      { w: q.w, x: q.x, y: q.y, z: q.z },
      true,
    );
  }
